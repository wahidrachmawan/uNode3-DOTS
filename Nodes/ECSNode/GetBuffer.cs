...
namespace MaxyGames.UNode.Nodes {
	...
	public class GetBuffer : ValueNode {
		...
		public override string GetRichTitle() {
			return $"Get Buffer: {type.GetRichName()}";
		}
			...
			if(executionKind == LookupExecutionKind.Auto) {
				...
				if(commandType == typeof(EntityManager)) {
					CG.RegisterUserObject<Func<string>>(() => {
						return CG.Invoke(typeof(SystemAPI), nameof(SystemAPI.GetBuffer), new[] { type.type }, entity.CGValue());
					}, ("ecb", this));
				}
		...
		protected override string GenerateValueCode() {
			if(executionKind == LookupExecutionKind.Auto) {
				...
				return typeof(SystemAPI).CGInvoke(nameof(SystemAPI.GetBuffer), new[] { type.type }, entity.CGValue());
			}
			else if(executionKind == LookupExecutionKind.EntityManager) {
				return entityManager.CGValue().CGInvoke(nameof(EntityManager.GetBuffer), new[] { type.type }, entity.CGValue());
			}
