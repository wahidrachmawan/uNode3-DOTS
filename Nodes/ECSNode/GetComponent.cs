...
namespace MaxyGames.UNode.Nodes {
	[NodeMenu("ECS/Data", "GetComponent", scope = NodeScope.ECSGraphAndJob, inputs = new[] { typeof(Entity) }, outputs = new[] { typeof(IComponentData) })]
	public class GetComponent : ValueNode {
		...
		public override string GetRichTitle() {
			return $"Get Component: {type.GetRichName()}";
		}
			...
			if(executionKind == LookupExecutionKind.Auto) {
				...
				if(commandType == typeof(EntityManager)) {
					CG.RegisterUserObject<Func<string>>(() => {
						return CG.Invoke(typeof(SystemAPI), nameof(SystemAPI.GetComponent), new[] { type.type }, entity.CGValue());
					}, ("ecb", this));
				}
		...
		protected override string GenerateValueCode() {
			if(executionKind == LookupExecutionKind.Auto) {
				...
				return typeof(SystemAPI).CGInvoke(nameof(SystemAPI.GetComponent), new[] { type.type }, entity.CGValue());
			}
			else if(executionKind == LookupExecutionKind.EntityManager) {
				return entityManager.CGValue().CGInvoke(nameof(EntityManager.GetComponentData), new[] { type.type }, entity.CGValue());
			}
			else if(executionKind == LookupExecutionKind.Lookup) {
				return lookup.CGValue().CGAccessElement(entity.CGValue());
			}
			return null;
		}
	}
